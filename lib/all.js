...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function onMouseDown(e) {
      ...
      if (gameEnded) {
        if (isInside(pos,buttonPositions['playagain'])){
          //TODO: Make play again functionality
          location.reload();
        }
      }else if (!gameOn) {
  ...
  function update() {
    ...
    console.log(latestNote,songNotes[songid[songNum]].length-1,latestNoteIndex)
    if (timer>latestNote[0]+2000 && last_note === true){
      gameOver();
    }
    let notesKeys = Object.keys(notes);
    if (latestNoteIndex >= songNotes[songid[songNum]].length-2){
      last_note = true;
    } else {
