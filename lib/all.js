...
document.addEventListener('DOMContentLoaded', () => {
  ...
  let songid = ["mammamia","trauma","mysea"]
  ...
  function onMouseDown(e) {
      ...
      for (let i = 0; i < notesKeys.length; i++) {
        ...
        if (note.x > drum.x - 90 && note.x < drum.x) {
  ...
  let life = 50;
  let startTime = 0;
  let latestNote = songNotes[songid[songNum]][0];
  ...
  function update() {
    ...
    let notesKeys = Object.keys(notes);
      if(timer + time_on_screen >= latestNote[0]){
        ...
        notes[note.id] = note;
        latestNoteIndex += 1;
		    latestNote = songNotes[songid[songNum]][latestNoteIndex+1];
      }
